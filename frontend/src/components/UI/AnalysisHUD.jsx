...
const AnalysisHUD = ({ isLight }) => {
    ...
    // Force show alts if confidence is too low (e.g. < 80%)
    const showAlt = keyInfo.alts.length > 0 && (keyInfo.conf < 80 || (keyInfo.conf - (keyInfo.alts[0]?.conf * 100 || 0)) < 20);
            ...
            <div className="flex flex-col items-end space-y-2 mb-4 pointer-events-none">
                <div className="flex items-start gap-3">
                    <div className="text-right flex flex-col items-end">
                        ...
                        <div className="text-3xl font-sans tabular-nums font-bold transition-all duration-100 flex items-center justify-end gap-2 leading-none mt-1">
                        ...
                        {chordData.aliases && chordData.aliases.length > 1 && (
                            <div className={`flex flex-col items-end gap-0.5 mt-2 animate-in slide-in-from-top-2 fade-in duration-300`}>
                                ...
                                {chordData.aliases.slice(1, 4).map((alias, idx) => (
                                    <span key={idx} className={`text-[10px] font-sans tabular-nums opacity-60 border-b border-dashed border-white/10`}>
                                        {alias}
                                    </span>
                                ))}
                            </div>
            ...
            <div className={`flex flex-col items-end space-y-2 border rounded-xl p-4 transition-all duration-500 ${panelBg} ${borderCol} pointer-events-auto`}>
                ...
                {isAnalyzing ? (
                    ...
                    <div className="flex items-center gap-6 opacity-90 animate-in fade-in slide-in-from-right-4 duration-500">
                        <div className="flex flex-col items-end"><span className={`text-[9px] ${textDim}`}>BPM</span><span className="font-bold font-sans tabular-nums tabular-nums">{dynamicStats.bpm}</span></div>
                        <div className={`w-px h-6 ${borderCol}`}></div>
                        <div className="flex flex-col items-end"><span className={`text-[9px] ${textDim}`}>METER</span><span className="font-bold font-sans tabular-nums tabular-nums text-midi-accent">{dynamicStats.meter}</span></div>
                        <div className={`w-px h-6 ${borderCol}`}></div>
                        <div className="flex flex-col items-end">
                            <div className="flex items-center gap-2">
                                <span className={`text-[9px] ${textDim}`}>KEY</span>
                                <span className={`text-[9px] font-sans tabular-nums ${textDim}`}>{keyInfo.conf}%</span>
                            ...
                            {showAlt && keyInfo.alts[0] && (
                                <div className={`text-[9px] font-sans tabular-nums mt-0.5 flex items-center gap-1 ${textDim}`}>
