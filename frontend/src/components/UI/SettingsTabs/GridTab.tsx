        ...
        <div className="flex-1 p-6 space-y-8 overflow-y-auto">
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border border-transparent hover:bg-white/5 transition-all">
                    ...
                    <Switch.Root className={`w-10 h-6 rounded-full relative transition-colors cursor-pointer ${percussionSettings.enabled ? 'bg-midi-accent' : 'bg-white/10'}`} checked={percussionSettings.enabled} onCheckedChange={(c: boolean) => setPercussionSettings({ enabled: c })}><Switch.Thumb className={`block w-4 h-4 bg-white rounded-full shadow transition-transform translate-x-1 will-change-transform ${percussionSettings.enabled ? 'translate-x-5' : 'translate-x-1'}`} /></Switch.Root>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <Slider.Root className="relative flex items-center select-none touch-none w-full h-4" value={[percussionSettings.rows]} min={1} max={16} step={1} onValueChange={(v: number[]) => setPercussionSettings({ rows: v[0] })}><Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]"><Slider.Range className="absolute bg-midi-accent h-full rounded-full" /></Slider.Track><Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" /></Slider.Root>
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between text-[10px] font-bold text-white/70">
                        ...
                        <Slider.Root className="relative flex items-center select-none touch-none w-full h-4" value={[percussionSettings.cols]} min={1} max={32} step={1} onValueChange={(v: number[]) => setPercussionSettings({ cols: v[0] })}><Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]"><Slider.Range className="absolute bg-midi-accent h-full rounded-full" /></Slider.Track><Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" /></Slider.Root>
                    </div>
                </div>
                    ...
                    {renderEngine === 'three' && (
                        <div className="space-y-4 animate-in fade-in duration-300">
                            ...
                            <div className="space-y-2">
                                ...
                                <Slider.Root className="relative flex items-center select-none touch-none w-full h-4" value={[percussionSettings.positionY]} min={-15} max={10} step={0.5} onValueChange={(v: number[]) => setPercussionSettings({ positionY: v[0] })}><Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]"><Slider.Range className="absolute bg-midi-accent h-full rounded-full" /></Slider.Track><Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" /></Slider.Root>
                            </div>
                        </div>
                    )}
                    ...
                    {renderEngine === 'p5' && (
                        <div className="space-y-5 animate-in fade-in duration-300">
                            ...
                            <div className="space-y-2">
                                ...
                                <Slider.Root className="relative flex items-center select-none touch-none w-full h-4" value={[percussionSettings.p5CellSize || 40]} min={20} max={100} step={5} onValueChange={(v: number[]) => setPercussionSettings({ p5CellSize: v[0] })}><Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]"><Slider.Range className="absolute bg-midi-accent h-full rounded-full" /></Slider.Track><Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" /></Slider.Root>
                            </div>
                            <div className="space-y-2">
                                <div className="flex justify-between text-[10px] font-bold text-white/70">
                                ...
                                <Slider.Root className="relative flex items-center select-none touch-none w-full h-4" value={[percussionSettings.p5Spacing || 10]} min={0} max={50} step={2} onValueChange={(v: number[]) => setPercussionSettings({ p5Spacing: v[0] })}><Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]"><Slider.Range className="absolute bg-midi-accent h-full rounded-full" /></Slider.Track><Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" /></Slider.Root>
                            </div>
                        </div>
                    )}
