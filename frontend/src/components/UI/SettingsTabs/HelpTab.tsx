...
import { HelpCircle, User, Mail } from 'lucide-react'; // 添加了 User 和 Mail 图标
      ...
      <div className="pt-6 border-t border-white/10 space-y-4">
        ...
        <div className="bg-white/5 rounded-lg p-5 space-y-3 border border-white/5">
          <div className="flex items-center justify-between">
            ...
            <span className="font-sans tabular-nums font-bold">PurrNeko</span>
          </div>
          <div className="flex items-center justify-between">
...
const ShortcutRow = ({ label, keys }) => (
  <div className="p-3 rounded bg-white/5 flex justify-between items-center hover:bg-white/10 transition-colors">
    ...
    <div className="flex gap-1">
      {keys.map(k => (
        <kbd key={k} className="px-2 py-1 bg-black/40 border border-white/10 rounded font-sans tabular-nums text-xs font-bold text-midi-accent shadow-sm">{k}</kbd>
      ))}
    </div>
  </div>
);
