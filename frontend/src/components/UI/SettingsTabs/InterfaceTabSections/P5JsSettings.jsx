// frontend/src/components/UI/SettingsTabs/InterfaceTabSections/P5JsSettings.jsx
import React from 'react';
import useStore from '@/store/useStore';
import { ScanLine, Wind, Star, BrainCircuit, Hourglass, Timer, Maximize, ArrowUpDown, Scaling, AlignJustify, ArrowRight, Grid3X3, ArrowRightToLine } from 'lucide-react';
...
const P5JsSettings = () => {
    ...
    const currentP5Settings = p5Settings || {
        ...
        fadeStartRatio: 0.45 // 默认在 45% 处开始消失
    };
                ...
                {(currentP5Settings.pageTurnMode === 'meteor' || currentP5Settings.pageTurnMode === 'fade') && (
                    <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200 border-t border-white/10 pt-4 mt-4">
                        ...
                        {currentP5Settings.pageTurnMode === 'fade' && (
                            <div className="space-y-2">
                                <div className="flex justify-between text-[10px] font-bold">
                                    <span className="flex items-center gap-1 opacity-60"><ArrowRightToLine size={12} /> Shrink Start Point</span>
                                ...
                                <Slider.Root
                                    ...
                                    value={[currentP5Settings.fadeStartRatio !== undefined ? currentP5Settings.fadeStartRatio : 0.45]}
                                    min={0.0} max={0.9} step={0.05}
                                    onValueChange={(v) => setP5Settings({ fadeStartRatio: v[0] })}
                                >
                                    <Slider.Track className="bg-white/10 relative grow rounded-full h-[3px]">
                                        <Slider.Range className="absolute bg-midi-accent h-full rounded-full" />
                                    </Slider.Track>
                                    <Slider.Thumb className="block w-3 h-3 bg-white rounded-full shadow hover:scale-110 focus:outline-none" />
                                </Slider.Root>
                            </div>
                        )}
                        ...
                        <div className="pt-2 space-y-2">
                            <div className="flex justify-between text-[10px] font-bold mb-2">
                                <span className="flex items-center gap-1 opacity-80"><Star size={14} /> {currentP5Settings.pageTurnMode === 'meteor' ? 'Meteor Fade Curve' : 'Shrink Curve'}</span>
                            </div>
                            <BezierCurveEditor
