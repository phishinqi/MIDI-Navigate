...
const WidgetSettings = () => {
    ...
    const items = [
        { label: 'Timeline & Controls', state: showPlayerWidget, toggle: togglePlayerWidget, icon: Layout },
        { label: 'Analysis HUD', state: showAnalysisWidget, toggle: toggleAnalysisWidget, icon: Sliders }
    ];
        ...
        <div className="space-y-4">
            ...
            {items.map((item, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-transparent hover:bg-white/5 transition-all cursor-pointer" onClick={item.toggle}>
                    <div className="flex items-center gap-3"><item.icon size={18} className="opacity-50" /><span className="text-sm font-bold">{item.label}</span></div>
                    <div className={`w-8 h-4 rounded-full relative transition-colors ${item.state ? toggleBgOn : toggleBgOff}`}><div className={`absolute top-0.5 w-3 h-3 rounded-full shadow-sm transition-all ${toggleDot} ${item.state ? toggleClassOn : toggleClassOff}`} /></div>
                </div>
            ))}
        </div>
    );
