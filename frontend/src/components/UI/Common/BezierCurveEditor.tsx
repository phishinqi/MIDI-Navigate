...
const GetTForX = (aX: number, mX1: number, mX2: number) => {
  let aGuessT = aX;
...
const SampleAnimator = ({ curveValue }: { curveValue: number[] }) => {
  ...
  const easerRef = useRef(createBezier(curveValue[0], curveValue[1], curveValue[2], curveValue[3]));

  useEffect(() => {
    easerRef.current = createBezier(curveValue[0], curveValue[1], curveValue[2], curveValue[3]);
  }, [curveValue]);
...
interface BezierCurveEditorProps {
  ...
  presets?: { name: string; curve: number[]; isDefault?: boolean }[];
  onAddPreset?: (name: string, value: number[]) => void;
...
const BezierCurveEditor: React.FC<BezierCurveEditorProps> = ({
  ...
  const getLogicalCoords = (e: MouseEvent | React.MouseEvent) => {
  ...
  const handleSavePreset = () => {
    if (presetName.trim() && onAddPreset) {
  ...
  const handleDeletePreset = () => {
    ...
    if (presetToDelete && !presetToDelete.isDefault && onRemovePreset) {
