...
const PixiVisualizer = () => {
  ...
  const activeNotesRef = useRef<Map<string, PIXI.Graphics>>(new Map());
  ...
  useEffect(() => {
    const setup = async () => {
      ...
      const unsub = (useStore as any).subscribe(
        (state: any) => state.incomingMidiEvent,
        (event: any) => {
    ...
    setup().then(result => {
      if (result) {
        cleanup = () => {
          result.unsub();
          if (result.app) {
            result.app.destroy(true, { children: true, texture: true, baseTexture: true } as any);
          }
          gsap.killTweensOf("*");
        };
      }
  ...
  // --- handleMidiEvent ---
  const handleMidiEvent = (event: any, container: PIXI.Container) => {
    if (!event) return;
  ...
  const spawnNote = (id: string, x: number, velocity: number, container: PIXI.Container, note: number, channel: number) => {
    const app = appRef.current;
    if (!app) return;
    ...
    // Fix: direct blendMode assignment needs string in recent Pixi versions or mapping
