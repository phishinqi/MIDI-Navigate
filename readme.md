MIDI-Navigate
MIDI-Navigate 是一个功能强大的 Web 应用程序，旨在提供交互式的 MIDI 文件可视化、播放和实时音乐理论分析。无论您是音乐制作人、学生还是爱好者，都可以通过它以全新的视角探索和理解音乐。
...
⚙️ 工作原理
文件上传: 用户在前端上传一个 .mid 文件。

全局分析: 前端将文件发送到后端的 /api/v1/analyze/midi-theory 端点。后端使用 pretty_midi 和 mido 等库进行全局分析，返回调性、BPM、节拍等信息。

播放与调度: 前端使用 Tone.js 精确调度 MIDI 事件的播放。
...
实时分析: 在播放过程中，前端将当前时刻正在发声的音符数据发送到后端的 /api/v1/analyze/realtime-chord 端点。后端利用 musicpy 快速分析并返回和弦结果，前端再将结果显示在分析面板上。