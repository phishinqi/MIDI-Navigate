...
export class KeyDetector {
    ...
    private heldNotes: Set<number>;
    private profiles: KeyProfile[];
    private currentKey: KeyResult;
    ...
    noteOn(note: number, sensitivity: number = 1.0): void {
    ...
    update(dt: number, halfLife: number = 3.0): void {
...
export class ChordAnalyzer {
    ...
    getRoman(chord: ChordResult, keyRoot: number, scaleName: string): string {
        ...
        if (SCALES[scaleName]) {
            targetIntervals = SCALES[scaleName].intervals;
        } else if (scaleName === 'Major' || scaleName === 'Minor') {
            targetIntervals = SCALES_LOOKUP[scaleName];
        } else if (scaleName && scaleName.includes('Minor')) {
            targetIntervals = SCALES['Minor (Aeolian)'].intervals;
        } else {
        ...
        if (degreeIndex === -1) {
            ...
            if (targetIntervals.includes(flat)) {
                ...
                degreeIndex = targetIntervals.indexOf(sharp);
                accidental = 'b';
            }
        }
        ...
        suffix = map[nameSuffix] || (nameSuffix === 'm' || nameSuffix === '' ? '' : nameSuffix);
